...
interface ItemPedidoDAO {
    ...
    @Query("""
        SELECT marmitas.nome, itens_pedido.quantidade, itens_pedido.precoUnitario 
        FROM itens_pedido 
        INNER JOIN marmitas ON itens_pedido.marmitaId = marmitas.id 
        WHERE itens_pedido.pedidoId = :pedidoId
    """)
